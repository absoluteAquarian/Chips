...
namespace Chips.IO {
	internal class BytecodeFile {
		...
		private static readonly List<BytecodeFile> importedFiles = new();
		...
		public BytecodeFile(string file) {
			if (Path.GetExtension(file) != ".bchp") {
				ChipsCompiler.exceptions.Add(new(file, "File extension was not \".bchp\""));
				return;
			}
			...
			if (readersByFile.ContainsKey(file)) {
				ChipsCompiler.exceptions.Add(new(null, "File is already being read: " + file));
				return;
			}
		...
		private void Compile(BytecodeFile? importTarget = null) {
				...
				switch (member) {
					case BytecodeFileMember.AssemblyInfo:
						...
						switch (asmInfo) {
							case 0:
								if (asmNameDefined) {
									ChipsCompiler.exceptions.Add(new(SourceFile, "Duplicate assembly name token found"));
									break;
								}
								...
								if (!CodeGenerator.IsValidLanguageIndependentIdentifier(asmName))
									ChipsCompiler.exceptions.Add(new(SourceFile, "Assembly name was not a valid name identifier: " + asmName));
								break;
							case 1:
								if (stackSizeDefined) {
									ChipsCompiler.exceptions.Add(new(SourceFile, "Duplicate stack size token found"));
									break;
								}
								...
								break;
							default:
								ChipsCompiler.exceptions.Add(new(SourceFile, $"Unknown assembly information token: 0x{asmInfo:X2}"));
								break;
						}
						break;
					case BytecodeFileMember.GlobalVariable:
						...
						if (!definedGlobalVariables.Add(name)) {
							ChipsCompiler.exceptions.Add(new(SourceFile, "Duplicate global variable declaration: " + name));
							break;
						}
			...
			if (!definedGlobalFunctions.Contains("main")) {
				ChipsCompiler.exceptions.Add(new(SourceFile, "Global function \"main\" was not defined."));
				return;
			}
		...
		private static void CompileEntryPoint() {
			//Creates the static Main() method, which invokes Chips.Runtime.Sandbox.Main()
		}
		...
		private static void CompileEverything(BytecodeFile target) {
			...
			if (!CodeGenerator.IsValidLanguageIndependentIdentifier(typeName)) {
				...
				if (!CodeGenerator.IsValidLanguageIndependentIdentifier(typeName)) {
					ChipsCompiler.exceptions.Add(new(target.SourceFile, "Source file had a name which could not be compiled to an identifier: " + typeName[1..]));
					return;
				}
			}
		...
		private static void CompileGlobalVariables(BytecodeFile target, ModuleDefinition module, TypeDefinition type) {
			for (int i = 0; i < target.globalVariables.Count; i++) {
				...
				if (globalType is null) {
					ChipsCompiler.exceptions.Add(new(target.SourceFile, "Variables cannot have a \"null\" type"));
					continue;
				}
			...
			for (int i = 0; i < target.globalFunctions.Count; i++) {
				...
				if (target == targetFileForEntryPoint && function.name == "main") {
					...
					if (function.attributes != (MethodAttributes.Public | MethodAttributes.Static)) {
						ChipsCompiler.exceptions.Add(new(target.SourceFile, "Entry point \"main\" must have the \"pub\" and \"stat\" accessors"));
						continue;
					}

					if (target.SourceFile == function.name) {
						ChipsCompiler.exceptions.Add(new(target.SourceFile, "Function name cannot match file name without the extension"));
						continue;
					}
				}
